...
void KSyncSockTcp::Receive(mutable_buffers_1 buf) {
    //Syncronous receive is not supported
}

TcpSession* KSyncSockTcp::AllocSession(Socket *socket) {
    TcpSession *session = new KSyncSockTcpSession(this, socket);
...
KSyncSock::KSyncSock() : tx_count_(0), err_count_(0), run_sync_mode_(false) {
